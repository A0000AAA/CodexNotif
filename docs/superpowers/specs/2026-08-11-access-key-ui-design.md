...
在 Windows 客户端“服务器设置”中提供遮罩访问密钥输入，让普通用户无需运行脚本即可配置 `CODEXNOTIF_ACCESS_KEY`，同时继续满足密钥只存放于环境变量、不进入应用配置文件的安全约束。

## User Experience

- 服务器地址下方增加 `PasswordBox`，标签为“服务器访问密钥”。
- 密钥框永远不回填真实值；留空表示保留当前环境变量中的值。
...
- `ServerAccessKeyResolver.SaveForCurrentUser` 负责校验并写入 Windows User 与当前 Process 两个环境变量目标。
- `ReadOptional` 优先读取 Process 值，并回退读取 User 值，使当前界面和以后由 Codex 启动的通知进程都能取得配置。
...
- UI 使用遮罩输入且不回填密钥。
- 密钥不进入源代码、README 示例值、日志、异常详情、应用设置或 Git 历史。
- 公网服务器地址仍必须为 HTTPS，客户端仍禁止自动重定向。
- 服务端仍通过宝塔环境变量配置完全相同的 `CODEXNOTIF_ACCESS_KEY`。

## Verification

- 自测试覆盖合法/非法密钥校验，以及环境变量读取优先级的纯逻辑选择。
