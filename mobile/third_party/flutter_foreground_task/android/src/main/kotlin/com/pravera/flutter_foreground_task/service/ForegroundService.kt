...
class ForegroundService : Service() {
    companion object {
        ...
        private const val MONITOR_WATCHDOG_INTERVAL_MILLIS = 10_000L
    ...
    private val monitorWatchdogHandler = Handler(Looper.getMainLooper())
    private var monitorWatchdogStarted = false
    private var lastObservedProgressMarker = ""
    private var lastProgressAtMillis = 0L
    private var lastRecoveryAtMillis = 0L
    private val monitorWatchdogRunnable = object : Runnable {
        override fun run() {
            try {
                verifyMonitorHealth()
            } catch (error: Throwable) {
                Log.e(TAG, "Monitor watchdog tick failed.", error)
            } finally {
                if (monitorWatchdogStarted) {
                    monitorWatchdogHandler.postDelayed(this, MONITOR_WATCHDOG_INTERVAL_MILLIS)
                }
            }
        }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // HyperOS can recreate the sticky service through a path that only
        // updates notification data. The old process wake lock is already gone,
        // so every start command must explicitly acquire it again.
        acquireLockMode()
    ...
    private fun startMonitorWatchdog() {
        if (monitorWatchdogStarted) return

        val nowMillis = SystemClock.elapsedRealtime()
        val persistedContent = NotificationContent.getData(applicationContext)
        notificationContent = persistedContent
        lastObservedProgressMarker = monitorProgressMarker(persistedContent.text)
        lastProgressAtMillis = nowMillis
        lastRecoveryAtMillis = 0L
        monitorWatchdogStarted = true
        monitorWatchdogHandler.post(monitorWatchdogRunnable)
        Log.i(TAG, "Monitor watchdog started; marker=$lastObservedProgressMarker")
    }

    private fun stopMonitorWatchdog() {
        monitorWatchdogStarted = false
        monitorWatchdogHandler.removeCallbacks(monitorWatchdogRunnable)
    }

    private fun verifyMonitorHealth() {
        if (!::notificationContent.isInitialized || !::foregroundTaskData.isInitialized) return

        val nowMillis = SystemClock.elapsedRealtime()
        val persistedContent = NotificationContent.getData(applicationContext)
        notificationContent = persistedContent
        val progressMarker = monitorProgressMarker(persistedContent.text)
        val decision = MonitorWatchdogPolicy.decide(
            nowMillis = nowMillis,
            progressMarker = progressMarker,
            lastObservedProgressMarker = lastObservedProgressMarker,
            lastProgressAtMillis = lastProgressAtMillis,
            lastRecoveryAtMillis = lastRecoveryAtMillis,
        )

        when (decision) {
            MonitorWatchdogDecision.HEALTHY -> {
                lastObservedProgressMarker = progressMarker
                lastProgressAtMillis = nowMillis
                Log.i(TAG, "Monitor heartbeat advanced; marker=$progressMarker")
            }
            MonitorWatchdogDecision.WAITING -> {
                if (lastObservedProgressMarker.isEmpty() && lastRecoveryAtMillis == 0L) {
                    updateMonitorNotificationText("正在启动后台监听…")
                }
            }
            MonitorWatchdogDecision.RECOVER -> {
                lastRecoveryAtMillis = nowMillis
                lastProgressAtMillis = nowMillis
                updateMonitorNotificationText("正在恢复后台监听…")
                try {
                    acquireLockMode()
                    createForegroundTask()
                    Log.w(TAG, "Monitor heartbeat stopped; the Dart task was rebuilt.")
                } catch (e: Exception) {
                    Log.e(TAG, "Unable to rebuild the Dart monitoring task.", e)
                    updateMonitorNotificationText("监听异常 · 自动恢复失败")
                }
            }
            MonitorWatchdogDecision.STALE -> {
                updateMonitorNotificationText("监听异常 · 超过 90 秒未检查")
            }
        }
    }

    private fun monitorProgressMarker(text: String): String {
        return Regex("\\d{2}:\\d{2}:\\d{2}\\s*$")
            .find(text)
            ?.value
            ?.trim()
            .orEmpty()
    ...
    private fun updateMonitorNotificationText(text: String) {
        if (!::notificationContent.isInitialized || notificationContent.text == text) return

        notificationContent = notificationContent.copy(text = text)
        NotificationContent.updateData(
            this,
            mapOf(PrefsKey.NOTIFICATION_CONTENT_TEXT to text),
        )
        updateNotification()
    }
